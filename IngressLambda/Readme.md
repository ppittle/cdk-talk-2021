...
WORKDIR /src
COPY ["IngressLambda.csproj", "IngressLambda/"]
RUN dotnet restore "IngressLambda/IngressLambda.csproj"

WORKDIR "/src/IngressLambda"
COPY . .
RUN dotnet build "IngressLambda.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "IngressLambda.csproj" \
...
Execute unit tests
```
    cd "IngressLambda/test/IngressLambda.Tests"
    dotnet test
```

Deploy application
```
    cd "IngressLambda/src/IngressLambda"
